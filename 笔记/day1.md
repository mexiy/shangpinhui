...
将reset.css放入public中清除默认样式
然后再index.htmL中引入reset.css
...
$route:一般获取路由信息【路劲，query，params等】
...
路由传递参数先关面试题
    ...
    ## 3:params参数可以传递也可以不传递，但是如果传递是空串，如何解决？
     ...
     这样会导致路径错误，路径中没有的search，可以使用undefined
    ...
    5: 路由组件能不能传递props数据?
     可以：三种写法
    1布尔值写法
     在路由配置中加上props:true,可以传递params，只能传递params，可以在页面上直接用插值表达式接收
    2 对象
    props:{a:1,b:2}
    3 函数
    props:(route)=>{
