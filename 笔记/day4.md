...
我们在哪里初始化swiper实例呢？
在mounted中，由于数据请求是异步的，可能在数据没有完全获得的时候，就已经初识化了轮播图实例，导致轮播图出现错误

由于我们获取的轮播图数据存储在vuex中，所以我们可以监听bannerList的数据，来确定是否开始初识化轮播图实例（watch）
使用watch监听
//现在咱们通过watch监听bannerList属性的属性值变化
//如果执行handler方法，代表组件实例身上这个属性的属性已经发生了变化
//当前这个函数执行：只能保证bannerList数据有了，不能保证v-for已经执行结束了
所以我们要使用 watch+nextTick；数据监听：监听已有的数据变化

$nextTick:在下次DOM更新 循环结束之后 执行延迟回调 在 修改数据之后， 立即使用这个方法 ，获取更行后的Dom
$nextTick:可以保证页面中结构一定是有的，经常和很多插件一起使用
