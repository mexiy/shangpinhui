        ...
        <div class="InfoWrap">
          <div class="goodsDetail">
            ...
            <div class="support">
              ...
              <div class="supportArea">
                ...
                <div class="fixWidth">雷瑟守备 下路 三角草</div>
              </div>
            </div>
          </div>
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <!-- 定义列表 -->
              <dl v-for="(attrList, index) in spuSaleAttrList" :key="attrList.id">
                <dt class="title">{{ attrList.saleAttrName }}</dt>
                <dd changepirce="0" v-for="(attrValueList, index) in attrList.spuSaleAttrValueList"
                  :key="attrValueList.id" :class="{ active: attrValueList.isChecked == 1 }"
                  @click="changeActive(attrValueList, attrList.spuSaleAttrValueList)">
                  {{ attrValueList.saleAttrValueName }}</dd>
...
export default {
  ...
  computed: {
    ...
    //给子组件的数据,因为当skuInfo中数据还没有的时候，防止传入的是undefined，传入一个空数组
    skuImageList() {
      return this.skuInfo.skuImageList || [];
    }
  },
  methods: {
    //实现详情页用ischecked来控制选中项的高亮业务（排他）
    changeActive(saleAttrValue, attr) {
      //遍历全部售卖属性ischecked为零没有高亮了
    ...
    //利用输入框传入数值
    //我们要排除用户输入不是数字的情况，可以让用户输入的内容*1，若是输入的是非数字，*1后为NaN
    changeSukNum(event) {
      //用户输入的文本*1
      let value = event.target._value * 1
      //判断是否为NaN,或负数
      ...
      */
      ...
      try {
       ...
       /*  console.log(result) */
        ...
        //产品信息的数据,比较复杂的skuInfo，通过会话存储（不持久化）
