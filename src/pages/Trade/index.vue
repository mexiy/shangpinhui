  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(address, index) in addressInfo" :key="address.id">
        <span class="username " :class="{ selected: address.isDefault == 1 }" @click="changeDefault(addressInfo, address)">{{
    address.consignee
}}</span>
        <p>
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="(detailList, index) in detailArrayList" :key="detailList.id">
          <li>
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch('getTradeDetail')
  },
  computed: {
    ...mapState({
    ...
    ...mapState({
      tradeDetail: state => state.trade.tradeDetail
    }),
  ...
  methods: {
    ...
    async submitOrder() {
      //交易编码
      let { tradeNo } = this.tradeDetail
      let data = {
        "consignee": this.deliveryAddress.consignee,//收件人
        "consigneeTel": this.deliveryAddress.phoneNum,//号码
        "deliveryAddress": this.deliveryAddress.fullAddress,//地址
        "paymentWay": "ONLINE",
        "orderComment": this.msg,//买家留言
        "orderDetailList": this.detailArrayList//购物清单
      }
      //需要带参数
